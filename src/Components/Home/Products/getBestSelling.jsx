...
const getBestSelling = async () => {
    try {
        ...
        querySnapshot.forEach((doc) => {
            ...
            if (productData.category === 'Mais Vendidos' || productData.category2 === 'Mais Vendidos') {
                Products.push({
                    id: doc.id,
                    ...productData,
                });
...
export default function AllBestSelling() {
    const [Products, setProducts] = useState([]);

    useEffect(() => {
        getBestSelling().then((result) => {
    ...
    return (
        <>
            <TitleCategory>Mais Vendidos</TitleCategory>
            <DivBody>
